"""
Script to import products from CSV/data
Usage: python backend/scripts/import_products.py
"""
...
def import_products():
    ...
    try:
        ...
        for idx, product in enumerate(PRODUCTS_DATA, 1):
            ...
            if not category_id:
                print(f"{idx:3d}. ❌ {product_name} - Category not found: {category_name}")
                error_count += 1
                continue
            ...
            if existing.data:
                print(f"{idx:3d}. ⚠️  {product_name} - Already exists, skipping...")
                skipped_count += 1
                continue
            ...
            if result.data:
                print(f"{idx:3d}. ✅ {product_name}")
                added_count += 1
            else:
                print(f"{idx:3d}. ❌ {product_name} - Failed to create")
                error_count += 1
