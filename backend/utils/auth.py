...
async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)) -> User:
    ...
    try:
        ...
        # Reduced logging - only log errors or in debug mode
        ...
        try:
            ...
            # Reduced logging - only log in case of errors
...
def require_manager_or_admin(current_user: User = Depends(get_current_user)) -> User:
    """Require manager, sales, accountant, hr_manager, employee, or admin role"""
    ...
    if role_value not in ["admin", "manager", "sales", "accountant", "hr_manager", "employee"]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Requires manager, sales, accountant, hr_manager, employee, or admin role"
        )
    return current_user
