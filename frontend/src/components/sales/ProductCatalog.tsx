  ...
  const load = async () => {
    try {
      ...
      const [{ data: prodData, error: prodErr }, { data: catData, error: catErr }] = await Promise.all([
        supabase.from('products').select('id, name, price, unit, description, category_id, created_at, is_active').order('created_at', { ascending: false }),
          ...
          Object.entries(
            ...
            <div key={catName} className="border-b border-gray-200">
              ...
              <table className="min-w-full text-sm text-gray-900">
                <thead>
                  <tr>
                    <th className="px-3 py-2 text-left font-semibold w-64">Tên</th>
                    ...
                    <th className="px-3 py-2 text-left font-semibold w-28">Trạng thái</th>
                    <th className="px-3 py-2 text-right font-semibold w-32">Thao tác</th>
                  </tr>
                </thead>
                <tbody>
                  {list.map(p => (
            ...
            <div className="w-full max-w-2xl bg-white rounded-lg shadow-2xl border border-gray-200">
              ...
              <div className="p-4 grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
                <div className="md:col-span-4">
                  <label className="block text-sm font-medium text-gray-900 mb-1">Loại sản phẩm</label>
                  <select
                    value={editCat}
