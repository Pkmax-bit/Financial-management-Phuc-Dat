...
interface ExpenseColumnVisibilityDialogProps {
  isOpen: boolean
  onClose: () => void
...
const EXPENSE_COLUMN_LABELS = {
  ...
  expense_percentage: '% (đối tượng chi phí)',
  expense_quantity: 'Số lượng (đối tượng chi phí)',
  expense_unit_price: 'Đơn giá (đối tượng chi phí)',
  expense_amount: 'VND (đối tượng chi phí)'
}

export default function ExpenseColumnVisibilityDialog({ 
  isOpen, 
  onClose, 
  visibleColumns, 
  onToggleColumn, 
  onReset 
}: ExpenseColumnVisibilityDialogProps) {
      ...
      <div className="bg-white/95 backdrop-blur-sm rounded-lg shadow-lg border border-gray-200 w-80">
        ...
        <div className="p-3 bg-white/90">
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {Object.entries(EXPENSE_COLUMN_LABELS).map(([key, label]) => (
