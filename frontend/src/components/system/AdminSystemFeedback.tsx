...
import { Plus, Search, Settings, Bug, Lightbulb, Palette, Zap, FileText, Users, BarChart3, Filter, Upload, X, Image as ImageIcon, Loader2, MessageCircle, Send, User, Eye, Edit2, Trash2 } from 'lucide-react'
...
export default function AdminSystemFeedback() {
  ...
  const handleDeleteAttachment = async (attachmentId: string) => {
    ...
    try {
      const { data: { session } } = await supabase.auth.getSession()
      const res = await fetch(`/api/feedback/system/${form.id}/attachments/${attachmentId}`, {
        method: 'DELETE',
        headers: {
      ...
      if (res.ok) {
        ...
        await load() // Reload to update list
      }
    } catch (error) {
      ...
      if (selectedFiles.length > 0 && feedbackId) {
        ...
        try {
          ...
          const uploadPromises = selectedFiles.map(async (file) => {
            // Try direct upload to Supabase Storage first (bypasses backend restrictions)
            try {
              const fileExt = file.name.split('.').pop() || 'bin'
              const fileName = `${Date.now()}-${Math.random().toString(36).substring(7)}.${fileExt}`
              const filePath = `SystemFeedbacks/${feedbackId}/${fileName}`
              
              // Read file as ArrayBuffer
              const fileBuffer = await file.arrayBuffer()
              
              // Upload directly to Supabase Storage (try without content-type first for PDF)
              let uploadResult
              
              // Try without content-type first (may bypass restrictions)
              ...
              if (uploadResult.error) {
                // If that fails, try with content-type
                uploadResult = await supabase.storage
                  .from('minhchung_chiphi')
                  .upload(filePath, fileBuffer, {
              ...
              // Update feedback attachments in database via backend API
              const updateRes = await fetch(`/api/feedback/system/${feedbackId}`, {
                method: 'PUT',
                headers: {
                  'Content-Type': 'application/json',
                ...
                body: JSON.stringify({
                  attachments: [...(existingAttachments || []), attachmentInfo]
                })
              })
              
              if (!updateRes.ok) {
                // If update fails, try to delete uploaded file
                await supabase.storage.from('minhchung_chiphi').remove([filePath])
                const errorText = await updateRes.text()
                throw new Error(`Failed to update feedback: ${errorText}`)
              }
              
              return attachmentInfo
            } catch (directUploadError) {
              console.warn('Direct upload failed, trying backend API:', directUploadError)
          ...
          // Update existing attachments state
          setExistingAttachments([...(existingAttachments || []), ...results])
        } catch (error) {
          ...
          // Don't throw, allow form submission to complete
        } finally {
  ...
  const ReplyItem = ({ reply, feedbackId, depth = 0 }: { reply: Reply, feedbackId: string, depth?: number }) => {
                    ...
                    {it.attachments && it.attachments.length > 0 && (
                      <div className="mb-3">
                        <div className="flex items-center space-x-2 mb-2">
                          <span className="text-xs font-medium text-gray-700">Đính kèm ({it.attachments.length}):</span>
                        </div>
                            ...
                            attachment.type === 'image' ? (
                              ...
                              >
                                <img
                                  src={attachment.url}
                                  alt={attachment.name}
                                  className="w-20 h-20 object-cover rounded-lg border border-gray-200 hover:border-purple-400 transition-colors cursor-pointer"
                                />
          ...
          <div className="flex min-h-screen items-end justify-end p-4">
            ...
            <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto border-l-4 border-blue-500">
              ...
              <div className="p-6 space-y-6">
                ...
                {form.id && existingAttachments && existingAttachments.length > 0 && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Hình ảnh/tài liệu hiện có
                    </label>
                          ...
                          {attachment.type === 'image' ? (
                            <>
                              ...
                              <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-0 group-hover:bg-opacity-20 rounded-lg transition-opacity"
                              >
                                <ImageIcon className="w-5 h-5 text-white opacity-0 group-hover:opacity-100" />
                              </a>
                            </>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    {form.id ? 'Thêm hình ảnh/tài liệu mới' : 'Đính kèm hình ảnh/tài liệu'}
                  </label>
              ...
              <div className="flex items-center justify-end space-x-3 p-6 border-t border-gray-200 bg-gray-50">
                ...
                <button 
                  onClick={submit}
                  disabled={uploadingAttachments}
                  className="px-6 py-2 text-sm font-medium text-white bg-gradient-to-r from-purple-600 to-purple-700 rounded-lg hover:from-purple-700 hover:to-purple-800 transition-all duration-200 shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
                >
