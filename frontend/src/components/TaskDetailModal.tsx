...
export default function TaskDetailModal({ taskId, onClose, onUpdate }: TaskDetailModalProps) {
    ...
    useEffect(() => {
        const loadUser = async () => {
            try {
                ...
                if (!authUser) {
                    // Handle unauthenticated state if necessary, though modal usually implies auth
                    return
                }
    ...
    const handleDeleteGroup = async () => {
        ...
        try {
            ...
            alert(getErrorMessage(err, 'Không thể xóa nhóm'))
        }
    }
    ...
    const handleStopTimer = async () => {
        const activeTimer = taskData?.time_logs?.find(log => !log.end_time)
    ...
    if (loadingUser || loadingTask || !taskData) {
        return (
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-hidden" onClick={onClose}>
            <div
                className="bg-white w-full max-w-[95vw] h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-scale-in"
            ...
            >
                ...
                <header className="h-14 border-b border-gray-200 bg-white flex items-center justify-between px-6 shrink-0">
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2 text-sm breadcrumbs text-gray-500">
                            <span>Tasks</span>
                            <span>/</span>
                            <span className="font-medium text-gray-900 max-w-[300px] truncate">{task?.title}</span>
                        </div>
                    </div>
                            ...
                            <div className="pt-4 border-t border-gray-200">
                                <div className="flex items-center justify-between mb-4">
                                    ...
                                    <button
                                        ...
                                        className="p-1.5 bg-blue-600 text-white rounded-md hover:bg-blue-700 flex items-center gap-1 px-3"
                                    >
                                        <Plus className="h-4 w-4" /> <span className="text-sm font-medium">Thêm việc</span>
                                    </button>
                                </div>

                                <div className="space-y-3">
                                    {/* Render Sub-tasks */}
                                    {taskData?.sub_tasks?.map(subTask => (
                                        <div key={subTask.id} className="bg-white border border-gray-200 rounded-lg p-3 hover:shadow-sm transition-shadow">
                                            <div className="flex items-start gap-3">
                                                <div className={`mt-1 w-4 h-4 rounded border flex items-center justify-center transition-colors ${subTask.status === 'completed' ? 'bg-blue-600 border-blue-600' : 'border-gray-300'}`}>
                                                    {subTask.status === 'completed' && <Check className="h-3 w-3 text-white" />}
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center justify-between">
                                                        <h4 className={`font-medium text-sm truncate ${subTask.status === 'completed' ? 'text-gray-500 line-through' : 'text-gray-900'}`}>
                                                            {subTask.title}
                                                        </h4>
                                                        <div className="flex items-center gap-2 text-xs text-gray-500">
                                                            {subTask.due_date && (
                                                                <span className={`flex items-center gap-1 ${new Date(subTask.due_date) < new Date() && subTask.status !== 'completed' ? 'text-red-600' : ''}`}>
                                    ...
                                    {/* Existing Checklists (Legacy support) */}
                ...
                {
                    isEditingTask && (
                        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[60]" onClick={() => setIsEditingTask(false)}>
                ...
                {
                    isEditingGroup && (
                        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[60]" onClick={() => setIsEditingGroup(false)}>
                            <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
