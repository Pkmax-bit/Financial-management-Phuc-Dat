...
type NoteVisibility = 'private' | 'task' | 'group'
...
export default function TaskDetailModal({ taskId, onClose, onUpdate }: TaskDetailModalProps) {
    const [activeTab, setActiveTab] = useState<'general' | 'checklists' | 'chat' | 'notes' | 'time' | 'participants' | 'attachments'>('general')
    ...
    const [noteVisibility, setNoteVisibility] = useState<NoteVisibility>('task')
    ...
    const handleCreateNote = async () => {
        ...
        try {
            await apiPost(`/api/tasks/${taskId}/notes`, {
                content: newNote,
                visibility: noteVisibility
            })
            setNewNote('')
    ...
    const handlePinComment = async (commentId: string, isPinned: boolean) => {
        try {
            await apiPut(`/api/tasks/comments/${commentId}`, {
                is_pinned: !isPinned
            })
            loadTaskDetails()
        } catch (err) {
