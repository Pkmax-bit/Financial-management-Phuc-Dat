#!/usr/bin/env python3
"""
Test script for payment API endpoint
"""
...
def test_payment_api(token, invoice_id):
    """Test payment API endpoint"""
    print(f"\n💰 Testing payment API for invoice {invoice_id}...")
    ...
    # Test with query parameters (as expected by the API)
    payment_data = {
    ...
    try:
        # Test the PUT endpoint with query parameters
        ...
        if response.status_code == 200:
            ...
            print(f"❌ Payment API failed: {response.text}")
            return False
    except Exception as e:
...
def main():
    ...
    # Test payment API
    test_payment_api(token, invoice_id)
