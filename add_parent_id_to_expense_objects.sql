...
-- Insert sample data to demonstrate hierarchy
INSERT INTO expense_objects (name, description, amount, parent_id) VALUES
('Xưởng', 'Chi phí tổng của xưởng sản xuất', 0, NULL),
('Nguyên Vật liệu chính', 'Chi phí nguyên vật liệu chính', 5000000, (SELECT id FROM expense_objects WHERE name = 'Xưởng' LIMIT 1)),
('Nguyên Vật liệu phụ', 'Chi phí nguyên vật liệu phụ', 2000000, (SELECT id FROM expense_objects WHERE name = 'Xưởng' LIMIT 1)),
('Nhân công', 'Chi phí nhân công sản xuất', 3000000, (SELECT id FROM expense_objects WHERE name = 'Xưởng' LIMIT 1));
